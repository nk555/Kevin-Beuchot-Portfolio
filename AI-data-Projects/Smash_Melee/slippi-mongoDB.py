...
def slp_to_mongoDB(path, db, state_dict={}, index=0, print_every=None, stop_at=None, start_at=0):
    ...
    for f in slp_files:
        ...
        if game_json["duration"]<600: #skip if game was exited at start (give 10 sec)
            continue
        try:
...
def slp_game_to_json(game, player_ports, index=0):
    ...
    game_json["stage"]=str(game.start.stage).split(".")[1].lower()
...
if __name__=="__main__":
    ...
    parser.add_argument("--database_uri", "-d", type=str, dest="db_uri", help="URI for MongoDB database.")
    ...
    parser.add_argument("--database_name", "-dn", type=str, dest="db_name", default="slippi", help="Database name inside MongoDB")
    ...
    db=client.__getattr__(args.db_name)
